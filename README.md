...
IRIS is a custom x86_64 operating system built from scratch with UEFI boot, ring 0 / ring 3 separation, and a capability-based microkernel architecture.

Zero userland code is compiled into the kernel binary. Every user process — `init`, `svcmgr`, `kbd`, `vfs` — is a standalone ELF service loaded from an embedded initrd at boot. Hardware resource policy (which service owns which IRQ or I/O port range) lives entirely in `svcmgr`, not in the kernel.

---

## Current Status

The system boots cleanly and exits with code 0.
...
- `KVmo` virtual memory objects: `SYS_VMO_CREATE` → `SYS_VMO_MAP` → `SYS_VMO_UNMAP` lifecycle
...
- Userland fault isolation: ring-3 exceptions (page fault, GP, etc.) kill the offending task instead of panicking the kernel; NMI / #DF / MCE remain fatal unconditionally
- IRQ routing owned by `svcmgr`: routes are process-scoped and auto-cleared on process exit
- `SYS_WAIT_ANY`: block until any one of up to 8 channels has a message (multiplexed recv)
...
| **Scheduling** | Timer-driven round-robin (100 Hz) with explicit blocking for IPC, notifications, IRQ, and sleep |
...
| **Process model** | `KProcess` owns address space and handle table; `struct task` owns CPU context and scheduler state |
...
|---|---|
| Bootstrap delivery to `svcmgr` | Kernel (explicit handle in spawn) |
...
| Userland fault handling | Kernel (kills faulting task; never panics on ring-3 exceptions) |
...
Kernel:
  ...
  3. Kernel → loads svcmgr from initrd, task_spawn_elf
  4. Kernel → delivers KBootstrapCap (SPAWN_SERVICE | HW_ACCESS) to svcmgr over bootstrap channel
  5. Kernel → loads init from initrd, task_spawn_elf
  6. Kernel → delivers svcmgr bootstrap channel handle to init (in RBX)
  7. Scheduler starts

User-space:
  8.  svcmgr → receives KBootstrapCap
  9.  svcmgr → requests KIrqCap(IRQ1) via SYS_CAP_CREATE_IRQCAP (HW_ACCESS gate)
  10. svcmgr → requests KIoPort(0x60–0x64) via SYS_CAP_CREATE_IOPORT (HW_ACCESS gate)
  11. svcmgr → SYS_INITRD_LOOKUP("kbd") → entry handle → SYS_SPAWN_ELF → spawns kbd
  12. svcmgr → SYS_INITRD_LOOKUP("vfs") → entry handle → SYS_SPAWN_ELF → spawns vfs
  13. svcmgr → delivers KIoPort + KIrqCap to kbd over bootstrap channel
  14. kbd    → flushes PS/2, registers IRQ1 route, signals ready
  15. vfs    → seeds boot exports (iris.txt, services.txt), signals ready
  16. svcmgr → signals ready
  17. init   → looks up kbd and vfs via svcmgr, exercises all service paths, exits 0
```
...
| `KOBJ_BOOTSTRAP_CAP` | Authority token for `SYS_CAP_CREATE_IRQCAP`, `SYS_CAP_CREATE_IOPORT`, `SYS_INITRD_LOOKUP` |
...
| 17 | `SYS_VMO_MAP` | Live | Map VMO pages into address space |
...
| 29 | `SYS_PROCESS_WATCH` | Live | Register process-exit watch on a channel |
| 30 | `SYS_DIAG_SNAPSHOT` | Live | Compact kernel-state snapshot into user buffer |
| 31 | `SYS_SPAWN_SERVICE` | Live | Legacy: name+auth → proc handle; prefer 41+42 |
...
| 35 | `SYS_PROCESS_KILL` | Live | Requires `RIGHT_MANAGE`; idempotent |
...
| 44 | `SYS_WAIT_ANY` | Live | Block until any of up to 8 channels has a message |
...
- Bootstrap handle delivery (kernel → svcmgr, svcmgr → services, kernel → init)
...
- IRQ-driven scancode delivery over `KChannel`
...
| `KCHANNEL_WAITERS_MAX` | 32 (= TASK_MAX) |
| `KNOTIF_POOL_SIZE` | 64 |
...
```
[IRIS][SVCMGR] service manager spawned, id=1, bootstrap_handle=1024
[SVCMGR] started
[SVCMGR] service spawned      ← kbd
[SVCMGR] service spawned      ← vfs
[SVCMGR] ready
KBD ready
VFS ready
...
kernel/core/syscall/syscall.c            Syscall dispatcher (syscalls 0–44)
kernel/core/scheduler/scheduler.c        Scheduler and task lifecycle
kernel/core/init/svcmgr_bootstrap.c      Kernel bootstrap: delivers KBootstrapCap to svcmgr and init
...
kernel/include/iris/syscall.h            Syscall ABI (0–44)
...
kernel/include/iris/service_catalog.h    Declarative service catalog (autostart, restart limits)
...
services/init/main.c + entry.S          Ring-3 init process
