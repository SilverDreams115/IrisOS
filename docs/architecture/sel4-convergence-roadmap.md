...
| No ambient authority | **close** | boot authority is one capability per authority, every per-process quota is gone (Stage 7), and the kernel's hardcoded ioport whitelist is REMOVED (Stage 5): the range a holder may claim travels on the `IOPORT_CONTROL` capability, narrowed by derivation (`SYS_IOPORT_CONTROL_NARROW`, T164/T171).  The kernel decides no device policy at all.  A-18 removed the last ambient authority but one: `SYS_VSPACE_SELF` and `SYS_CSPACE_SELF` handed a thread its own address space and CSpace asking for no capability at all, and are RETIRED — delegated at `IRIS_CPTR_OWN_VSPACE`/`OWN_CSPACE` for services and in BootInfo for the root task, which is seL4's arrangement.  `SYS_TCB_SELF` remains, because a thread's own TCB is per-THREAD and delegation needs a per-thread channel that does not exist yet; what it would take is recorded rather than waved at |
