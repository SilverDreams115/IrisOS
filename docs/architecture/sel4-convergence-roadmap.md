...
| MCS scheduling | **mostly there** | budget and period are enforced; **timeout faults** landed in Stage 8-mcs (`SYS_TCB_SET_TIMEOUT_HANDLER`, T307), so an overrun is a policy decision a temporal supervisor makes rather than an invisible stall; and **sporadic replenishment** landed with them, so every tick consumed comes back exactly one period later and a thread can never spend more than its budget in any window of its period (host R-1..R-8).  Still missing: **SC donation over IPC**, which is what a passive server needs |
