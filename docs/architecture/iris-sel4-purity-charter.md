...
| The message is a struct, not message registers | A message is `struct IrisMsg` — 80 bytes in user memory, named by a pointer — where seL4's is a `MessageInfo` word plus message registers plus the IPC buffer.  Split out of the ABI row by A-32, because it is a divergence about how a MESSAGE is carried rather than about how a METHOD is named, and the two closed separately: the buffer half is D-4, closed at Stage 8-cap.  Retiring the struct is 339 ring-3 sites and 130 kernel references, the same size as the whole invocation conversion, and buys nothing this charter measures beyond what D-4 already bought | Permanent, deliberate |
