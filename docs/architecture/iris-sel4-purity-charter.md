...
| A5 | No ambient authority exists | **PARTIAL — one call, found by the A-20 audit.**  The ioport whitelist went in Stage 5, every per-process quota went in Stage 7, and the three SELF syscalls were retired in A-18 (a service is given its own VSpace/CSpace/TCB before its first instruction; the root task finds them in BootInfo; a thread the loader never saw is handed its TCB by its creator in the entry register).  What A-18 did not look at is `SYS_THREAD_PRIORITY`: it sets the CALLER's own priority, up to 255, **taking no capability at all** — the same shape as the SELF syscalls, on the scheduler instead of on the CSpace.  `SYS_TCB_SET_PRIORITY` is the capability-based call that already does this properly |
