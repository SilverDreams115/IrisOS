...
| IPC capability transfer is a MOVE | Sending a capability over an endpoint EMPTIES the sender's slot; seL4 COPIES, and the sender keeps its capability, gated by the Grant right.  Both are coherent and IRIS's is strictly the more conservative — nothing can be delegated without the delegator giving it up — but it is a real difference in what an endpoint is, and a client that wants to keep what it sends derives a copy per send.  Found and written down in ledger A-26; it had been the behaviour since Stage 2 and was never stated | Permanent, deliberate |
