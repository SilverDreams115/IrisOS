...
| A5 | No ambient authority exists | PARTIAL — the ioport whitelist is what remains (ledger).  **Every per-process quota is gone**: the notification quota in Phase S1, the PAGE quota and the live-process ceiling in Stage 7 Steps 2-3, and the VMO-count quota with the owner relation in Stage 7-mem.  The page quota went because a VMO's pages come from an Untyped the caller named and a second ceiling the kernel invented contradicted that rather than reinforcing it; the others went for the same reason.  **Stage 5**: boot authority is fine-grained and NAMED — six capabilities, one authority each, matched by exact equality, and a monolithic boot capability cannot be constructed (`kbootcap_alloc` refuses a multi-bit kind); `SYS_BOOTCAP_RESTRICT` is retired.  Improved in Phase S4: device caps (KIrqCap/KIoPort) are no longer fabricated into a handle table — they are published into CSpace as MDB children of the authorising bootstrap-cap slot, so device authority is traceable and revocable.  The KDEBUG scan is GONE (Step 4): `SYS_KLOG_DRAIN`/`SYS_SCHED_INFO`/`SYS_POWEROFF` used to search the caller's handle table for any bootstrap cap bearing KDEBUG — the caller named nothing — and now require the capability as a CPtr argument, with no fallback |
...
| S3 | The SC represents a delegable time budget/policy | MET (budget/period enforced; **timeout faults landed Stage 8-mcs** — an overrun is delivered to a temporal supervisor that decides, rather than stalling invisibly.  Donation over IPC still pending — Stage 8) |
...
| P2 | The kernel implements mechanism, not product policy | PARTIAL — the ioport whitelist is the last of it in the kernel (ledger).  The per-process quotas it used to name went with the resource domain in Stage 7 |
