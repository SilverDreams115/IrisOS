...
| # | Invariant | Today |
|---|---|---|
| M1 | Frames, page tables, VSpace converge to creation from Untyped | **MET for creation, PARTIAL for form** — Stage 6: page tables, PML4s, VSpace and KProcess headers, root CNodes, VMO pages and metadata, mapping records and device capabilities are all carved from an Untyped somebody named, and each is a child of it so the region cannot be reset under live objects.  What is not seL4's form is that the KERNEL carves them on the caller's behalf rather than the user retyping and mapping each one (ledger D-5, Stage 7).  Kernel-funded after Stage 6: only the root task, built before any Untyped exists |
