...
| P2 | The kernel implements mechanism, not product policy | **MET (ledger A-19).**  Removed: the kernel FUTEX (a synchronization product — 176 lines, a 256-waiter ceiling, keyed on a raw user address rather than a capability; seL4 has none), the notification waiter ceiling (`KNOTIF_WAITERS_MAX` = 4, while endpoints already queued intrusively through the TCB), the default CNode size (0 meant 256 slots the caller never asked for), two dead scheduling defaults, and **the thread ceiling**: `ktcb_registry[TASK_MAX]` refused a thread when its array filled, and everything reading it was walking it — it is an intrusive list now, and the static pool shrank from 256 to the bootstrap pair.  Classified as mechanism and left: resolver and object bounds, the retype batch's IRQ-off window, the PIC's 16 lines, seL4's own priority range and time slice, the panic serial |
