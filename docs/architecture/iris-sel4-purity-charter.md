...
| Own ABI (not seL4) | IRIS does not seek binary compatibility.  Concretely: numbered syscalls, each taking CPtrs and checking rights itself — where seL4 has a handful and expresses every other operation as an INVOCATION on a capability carrying a method label.  **This is the LAST of the four form divergences A-20's audit named, and the only one still open — the other three are closed (A-21 address-space identity, A-22 faults as IPC, A-24 timed blocking), so what is left here is a decision rather than a gap.**  The authority semantics are equivalent and that is what the invariants are about: nothing is reachable without naming a capability, every operation checks rights on the object it acts on, and the syscall number selects a METHOD, never an object.  What an invocation ABI would buy is a smaller kernel entry surface and one place to check authority instead of sixty; what it would cost is rewriting every caller in the system to gain nothing this charter measures.  The honest statement is that IRIS chose a different shape for the same semantics, and says so here rather than pretending the shape does not differ | Permanent, deliberate |
