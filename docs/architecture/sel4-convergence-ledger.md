...
| D-9 | **IRIS creates no DEVICE Untypeds at all — and the code that would serve them takes its object headers from the kernel slab** | Two facts, and the second only matters because of the first.  (1) `kuntyped_create` is called from exactly two places: boot, which passes `is_device = 0` for every region it hands the root task, and `retype_sub_untyped`'s device branch, which requires an already-device Untyped.  Nothing ever sets the flag, so **no device Untyped can exist**, and invariants U11/U12 ("a device Untyped produces only UNTYPED/FRAME") describe an object the system cannot construct.  (2) Those unreachable branches take the REGION from the device (correctly) and the object HEADER from `kslab`, which the RAM path stopped doing at Stage 6 Step 1 | seL4's BootInfo lists device Untypeds alongside RAM ones — that is how a driver is handed an MMIO region as a capability and retypes frames from it.  seL4 also has no object headers, so the second fact has no counterpart there | A driver cannot be given MMIO as an Untyped, so IRIS reaches device memory another way: `SYS_FRAMEBUFFER_VMO` fabricates a KVMO over it, which is the object family D-5 still records as un-retyped.  The two gaps are the same gap seen from opposite ends.  The header issue is not live TODAY — nothing can reach it — and becomes live the moment the first fact is fixed, which is exactly when it must be solved rather than discovered | **RECORDED, NO STAGE.**  Sequenced with the memory server (D-5), because "the framebuffer is a KVMO" and "there are no device Untypeds" are one problem: the fix for both is boot publishing MMIO regions as device Untypeds in BootInfo, and a driver retyping frames from the one it is given.  Doing that requires answering the header question first — a device region is MMIO, not RAM, so a header cannot be carved from the region it describes, and `SYS_UNTYPED_RETYPE2` has no argument left to name a RAM budget with (`ut`, `type|count`, `dest`, `obj_arg`).  The alternative is seL4's: no headers.  **Found by the purity gate's new reachability check, in the same change that added it**, and the first draft of this row was WRONG — it said a ring-3 holder could reach the allocation, which is true of the code and false of the system, because nothing can hold what cannot be created |
