...
|---|---|---|---|---|---|---|
| `KProcess` | process as a kernel object = policy in the kernel | spawn/loader, supervision, fault info, accounting | user-space process server (TCB+CNode+VSpace+Untyped) | process-server | yes | FROZEN — **Stage 7 SUBSTITUTED one consumer, it did not add one.**  `SYS_TCB_CONFIGURE` gained a process argument (arg3; 0 still means "my own") in the same change that retired `SYS_THREAD_START`, which consumed `KProcess` for the same authority.  The surface shrank by a syscall and `task_thread_create` went with it.  Recorded because the frozen rule reads "no new consumers", and a reviewer running it against this diff should see the trade rather than have to reconstruct it.  The page quota and the live-process ceiling it carried are retired (Stage 7 Steps 2-3).  **Stage 7 Steps 4-6 took the hot path off it entirely**: a thread holds its own CSpace root and its own VSpace — the two capabilities `SYS_TCB_CONFIGURE` already named — so resolving a CPtr and switching CR3 no longer read `KProcess`, and the fault record moved to the thread that takes it.  `user_cr3` and `pcid` are gone (the tag belongs to the walk); `cr3` survives as a declared cache for teardown.  What remains on the object is exactly the SUPERVISION policy — fault and exit notification, exit code, thread count, the kernel-side budget, and the `SYS_PROCESS_*` surface — which is the process server's job description.  The VMO-count quota and the object itself go with it |
...
| root CNode at `kprocess_alloc` (kslab) | runtime CNode outside Untyped | the ROOT TASK only | the spawner supplies a retyped CNode (process-server) | Stage 7 | yes | **MIGRATING (Stage 6 Step 4)** — a spawned process's root CNode is a top-carved child block of the budget its spawner named, as is the `KProcess` itself.  What remains on the slab is the root task, built before any Untyped exists.  The remaining step is seL4's shape: the spawner RETYPES the CNode and passes it, which needs user-space process composition (Stage 7) |
...
| process-level fault record (one per process) | belongs on the TCB | fault delivery (Phase 20/25) | per-TCB fault / fault EP | process-server | yes | ACTIVE_LEGACY |
