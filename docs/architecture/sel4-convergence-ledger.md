...
| D-9 | **A DEVICE untyped's object headers come from the kernel slab** | Retyping a `KOBJ_FRAME` or a sub-`KOBJ_UNTYPED` out of a device Untyped takes the region from the device (correctly) and the OBJECT HEADER from `kslab` (`kframe_alloc`, `kuntyped_create`).  The RAM path does not: since Stage 6 Step 1 a RAM retype carves the header from the same Untyped, out of the opposite end | seL4 has no object headers at all — a capability IS the object, and its bookkeeping lives in the CNode slot — so the question does not arise there | A ring-3 holder of a device Untyped can make the kernel allocate.  It is BOUNDED — each retype consumes region space, so the header count cannot exceed `region_size / PAGE_SIZE` — but for a large MMIO region that is thousands of headers, and none of them is charged to anybody.  It is also the reason the purity gate's occurrence counts said nothing useful: they froze a number per file and could not see who could reach it | **RECORDED, NO STAGE — and the reason it has no obvious fix is worth stating.**  A device region is MMIO, not RAM, so the header cannot be carved from the region it describes; that is the whole difference from the RAM path.  The clean answer is for a device retype to NAME a RAM budget for its headers, which `SYS_UNTYPED_RETYPE2` has no argument left for (`ut`, `type|count`, `dest`, `obj_arg`).  The other answer is the one seL4 took: no headers.  Both are real changes, neither is small, and pretending the current state is fine because the bound exists would be the process failure charter §5.1 forbids.  **Found by the purity gate's new reachability check, in the same change that added it** — the check was written because a syscall had just put `kslab_alloc` one call from ring 3, and the first thing it did was find a second case nobody had recorded |
