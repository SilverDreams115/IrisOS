...
| D-5 | **Memory objects are CHARGED to an Untyped, not RETYPED by the user** (Stage 6; PAGE TABLES and the ADDRESS SPACE converted in Stage 6-pure) | The kernel carves VMO pages and mapping records out of an Untyped the caller NAMED, and accounts each as a child of it | The user retypes each object explicitly and maps it (`seL4_X86_PageTable_Map`); frames, IRQ handlers and I/O ports have no kernel object at all | Accounting and revocation are equivalent — nothing is created without a budget, and a budget cannot be reset while its objects live — but the user does not choose WHEN or WHERE each level exists, and cannot hold a capability to an individual page table | **CLOSED for address spaces, CSpaces, threads and the FRAMEBUFFER (Stage 6-pure, Stage 7, Stage 6/D-9); what remains is the KVMO's other uses, and retires with the memory server.**  The framebuffer was the last physical region the kernel wrapped in an object of its own making (`kvmo_wrap`, called only from `SYS_FRAMEBUFFER_VMO`); it is a DEVICE Untyped now and `fb` retypes its own frame from it, so both the syscall and the wrapper are deleted.  The INITRD images followed: `SYS_INITRD_FRAME` hands a boot image over as a FRAME carved from the caller's budget and answers its size, one map covers it (D-10), and `SYS_INITRD_VMO` is retired with its code deleted — a second way to obtain a boot image that nothing tests is worse than none.  The loader and vfs stopped speaking a second memory ABI to read a file the kernel already had.  The LOADER followed: a spawned image's segments and its user stack are FRAMES retyped from the child's budget, mapped whole (D-10) into the loader's window to fill and then into the child with W^X flags.  Every service in the system is now loaded without a KVMO existing at any point.
...
**What is left is ONE path**: `PGR_OP_MAP_RESUME`, which maps a page of a region the CLIENT granted, at an offset the client names.  That page-at-a-time shape over somebody else's multi-page region is the one thing a VMO does that a frame does not, and the frame answer — one capability per page, granted by the client — is not a smaller change: it redefines what a grant IS, and twenty tests are written against the shape it replaces.  Recorded rather than rushed.
