...
| I-20 | No demand paging exists in the kernel (`#PF` ring-3 never allocates pages) | the `#PF` handler records the fault on the faulting THREAD and signals the notification its handler was armed with; it allocates nothing | FR-41 | silent demand reintroduction |
...
| # | Invariant | Implementation | Tests | Risk if broken |
|---|-----------|----------------|-------|----------------|
| B-01 | The kernel creates no paging level for a user address space | `kframe_map_page` maps strictly (`paging_map_strict_in`) and answers `IRIS_ERR_MISSING_TABLE`; the holder retypes an `IRIS_KOBJ_PAGE_TABLE` and installs it with `SYS_VSPACE_MAP_TABLE` | T302, PT-1..PT-8 | the kernel funds user mappings again (charter M3) |
| B-01b | The one exception is the root task's maps made BEFORE it runs, and it ENDS | `KVSpace.kernel_funded`, cleared by `kvspace_end_bootstrap` once the root task holds its Untypeds | PT-8 | the kernel keeps funding an address space that could pay for itself |
| B-02 | A page table, PML4, VSpace/CNode/TCB header, VMO page, mapping record or device capability is a CHILD of the Untyped it came from | `kuntyped_alloc_page_child` / `kuntyped_alloc_child_top`; for retyped objects the header block carries the entry | T298, T299, T300, T301 | `SYS_UNTYPED_RESET` reclaims a region whose pages are live, handing the same memory out twice |
| B-02b | An address space and a root CSpace are RETYPED by their holder and NAMED at `SYS_TCB_CONFIGURE`, which builds neither | `retype_vspace`, `RETYPE2(KOBJ_CNODE)`, `sys_tcb_configure` | T299, T301 | the kernel decides the shape of a process nobody asked it to |
...
| B-04 | Teardown returns page children, then the header block, then the pool retain | `kvspace_obj_destroy_ut`, `kvmo_destroy`, the TCB destructor | T299, T300 | the header block's parent is released while the block is still being read |
