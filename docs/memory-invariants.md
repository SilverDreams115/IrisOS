...
| B-02 | A page table, PML4, VSpace/KProcess/CNode header, VMO page, mapping record or device capability is a CHILD of the Untyped it came from | `kuntyped_alloc_page_child` / `kuntyped_alloc_child_top`; for retyped objects the header block carries the entry | T298, T299, T300, T301 | `SYS_UNTYPED_RESET` reclaims a region whose pages are live, handing the same memory out twice |
| B-02b | An address space and a root CSpace are RETYPED by their holder and handed to `SYS_PROCESS_CREATE`, which builds neither | `retype_vspace`, `RETYPE2(KOBJ_CNODE)`, `kvspace_bind` / `kcnode_bind_root` (both exclusive) | T299, T301 | the kernel decides the shape of a process nobody asked it to |
