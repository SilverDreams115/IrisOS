...
|---------|-----------|-----------------|----------------|
| `svcmgr` | 15 | Bootstrap delivery (handle-boundary caps only), dynamic registration, service activation, legacy main loop, legacy console writes | **Phase 8**: console.ep arrives as CSpace mint (kind 0x22 retired); mints slots 1–5/7 into every catalog child pre-start; kinds 0x20/0x21/0x23 retired; still the last legacy console **writer** |
| `vfs` | 1 | Bootstrap receive of INITRD_CAP only | **Phase 8**: own EP = slot 5, console = slot 3, no lookups; KBootstrapCap is the documented handle boundary |
| `sh` | **0** | — | **Phase 8 DONE** — pure CPtr-first client: empty bootstrap bag (endpoint_only, no own ep), slots 1–4, zero KChannel call sites |
| `init` | 58 | Console bootstrap; svcmgr channel; kbd legacy probes (S2/S7); IPC selftests; `.ep` spoof-register probe | **Phase 8**: pre-start mints into console (slot 5), svcmgr (slot 3) and iris_test (1–4, 30/31); kind 0x20 forward retired; remains the orchestrator and largest debtor |
| `console` | 4 | Bootstrap receive (KIoPort + service channel); legacy receive path (svcmgr writes + SYNC v2) | **Phase 8**: own EP recv = slot 5 (kind 0x21 retired); legacy path remains only for svcmgr |
...
| `kbd` | 5 | Legacy HELLO/STATUS/SUBSCRIBE probes (non-blocking drain), bootstrap (KIoPort/KIrqCap/channels) | **Phase 8**: own EP = slot 5, IRQ notification = slot 7 (kinds 0x21/0x23 retired); channel carries only probes + handle-boundary caps |

`iris_test` KChannel tests (T001–T012) are regression tests for the KChannel syscall layer; they are kept indefinitely.

## Coexistence rules (Phase 7 / 7.1 constraint)

1. `handle_id_t` and `KChannel` must not be removed until **all** production callers are migrated.
2. svcmgr creates its KEndpoint (`state->ep_h`) **before** `svcmgr_autostart_services` so every catalog service receives the ep cap in its bootstrap bag. Phase 7.1 adds per-service endpoints (`own_service_ep`, bootstrap kind 0x21) created lazily in `boot_service` and kept across restarts.
...
4. **Phase 7.2 update:** the VFS endpoint path is now *mandatory* for sh and init. sh prints `[SH] vfs ep FAILED` (and the smoke gate fails) when the lookup fails — there is no silent fallback. init exits with code 5 if `"vfs.ep"` cannot be resolved.
5. Both lookup paths (EP and legacy) must resolve `".ep"` names identically; dynamic registration of `".ep"` names is rejected (runtime-tested: init S4 register-side probe + iris_test T031 lookup-side probe). This includes `"console.ep"` (Phase 7.3), which is bootstrap-delivered by init (kind 0x22), never runtime-registered.
6. **Phase 8 update:** core service discovery is CPtr-first — the spawner pre-start-mints slots 1–4 (see `docs/cptr-first-services.md`); the kind-0x20 bootstrap cap is retired. Name lookup (`IRIS_SVCMGR_EP_LOOKUP_NAME`, legacy `SVCMGR_MSG_LOOKUP_NAME`) stays alive for dynamic services and as the redistribution path (init re-mints looked-up `.ep` caps into iris_test); its handle results stay first-class (T046). Mints are non-fatal; every CPtr consumer gates loudly.

## Debt classification

### Class A — svcmgr legacy path (Phase 7 debt)
...
### Class B — service bootstrap phase (Phase 8: largely replaced by CSpace mints)
...
### Class C — operational request/reply loops (Phase 7.1: VFS/SH migrated)

`vfs` and `sh` used KChannel for every file operation (`SYS_CHAN_SEND` +
`SYS_CHAN_RECV` on a reply channel). Phase 7.1 moved the operational path to
...
- **Client**: sh's `ls` and `cat` issue EP_CALLs against `"vfs.ep"`
  (endpoint-only since Phase 7.2 — the legacy fallback was removed and svcmgr
  no longer forwards the vfs service/reply channels to sh).
- **init (Phase 7.2)**: the S5/S6 healthy-path probes use the stateless EP
...
- **Remaining legacy Class C debt**: the VFS stateful channel protocol
  ...
  server loop in `vfs.c` stays alive purely as compatibility until Phase 7.5
  removes it (a stateful EP replacement stays blocked on sender identity).

### Class D — keyboard event channel (sh ← kbd: RESOLVED in Phase 7.4)
...
Class D residue after Phase 7.6:
- ~~kernel IRQ routing delivers `KBD_MSG_IRQ_SCANCODE` over KChannel~~ —
  **RESOLVED (Phase 7.6)**: `SYS_IRQ_ROUTE_REGISTER` now accepts a
...
### Class E — console write path (Phase 7.3: endpoint-first)
...
Phase 7.1 interleaving class of bugs cannot recur on this path). Remaining
legacy writer: svcmgr (retires with its legacy loop).

## Migration steps for a Class C service (procedure as executed for vfs, Phase 7.1)
...
7. Coexistence phase only: keep a runtime fallback while the endpoint path is being proven. Once the EP path is gated in smoke (next step), REMOVE the fallback — a surviving fallback masks endpoint regressions (this is what Phase 7.2 did for sh).
...
```
Phase A (complete):  svcmgr EP coexistence (Phase 7)
Phase B (complete):  vfs EP server — stateless LIST/STAT/READ_AT (Phase 7.1)
Phase C (complete*): sh vfs operations over EP (Phase 7.1)
                     *kbd event channel deferred to Phase G
Phase C2 (complete): init S5/S6 VFS probes over EP; sh fallback removed;
                     VFS legacy protocol left clientless (Phase 7.2)
...
## Phase 11 — REGISTER moved off KChannel
...
## Phase 12 — svcmgr legacy-loop retirement & classification

The svcmgr KChannel dispatch is now strictly classified (comment at the loop):

| KChannel path | Final state | Why it stays |
|---|---|---|
| ~~PROC_EVENT_MSG_EXIT~~ | **retired (Phase 13 / Track B)** | death is now a KNotification signal — see below |
...
Retired in Phase 12 (dead code): `SVCMGR_MSG_STATUS` handler and the
...
## Phase 13 — Track B: proc-exit watch → KNotification
